...
version=${VERSION:-$(curl -s "https://hub.docker.com/v2/repositories/mind-developer/kvoip-v2/tags" | grep -o '"name":"[^"]*"' | grep -v 'latest' | cut -d'"' -f4 | sort -V | tail -n1)}
branch=${BRANCH:-$(curl -s https://api.github.com/repos/mind-developer/kvoip-v2/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
curl -sLo docker-compose.yml https://raw.githubusercontent.com/mind-developer/kvoip-v2/$branch/packages/twenty-docker/docker-compose.yml

# Copy twenty/packages/twenty-docker/.env.example to .env
echo -e "\t• Setting up .env file"
curl -sLo .env https://raw.githubusercontent.com/mind-developer/kvoip-v2/$branch/packages/twenty-docker/.env.example
