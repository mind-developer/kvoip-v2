...
import ActiveCallRingManager from '../utils/ActiveCallRingManager';

export const useActiveCallRing = (isRinging: boolean, isActiveCall: boolean) => {
  const managerRef = useRef<ActiveCallRingManager | null>(null);

  useEffect(() => {
    // Cria uma única instância do manager
    if (!managerRef.current) {
      managerRef.current = new ActiveCallRingManager();
    }
