...
export const SettingsDataRoleOverview = () => {
  ...
  const { roles, loading, error, fetchRoles } = useMockRole();
  ...
  useEffect(() => {
    const nodesFromRoles = roles.map((role, index) => {
      const existingNode = nodes.find((node) => node.id === role.id.toString());
      ...
      return {
        ...
        position: existingNode?.position || {
          ...
          y: index % 2 === 0 ? startY : startY + 100,
        },
      };
    });

    setNodes(nodesFromRoles);
  }, [roles]);
